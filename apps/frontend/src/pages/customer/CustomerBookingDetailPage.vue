<template>
  <div class="space-y-4">
    <RouterLink :to="`/customer/${slug}/bookings`" class="text-sm text-gray-500 hover:text-gray-700">← Portal Pelanggan</RouterLink>

    <AppSpinner v-if="loading" class="mx-auto" size="lg" />

    <div v-else-if="booking" class="space-y-4">
      <div class="flex items-center gap-3">
        <h2 class="text-lg font-semibold text-gray-900">{{ booking.asset_name ?? 'Pemesanan' }}</h2>
        <AppBadge :status="booking.status" type="booking" />
      </div>

      <AppCard title="Detail Pemesanan">
        <dl class="space-y-3 text-sm">
          ...
          <div class="flex justify-between"><dt class="text-gray-500">DP dibayar</dt><dd>{{ formatCurrency(booking.upfront_fee) }}</dd></div>
        </dl>
      </AppCard>

      <AppCard v-if="booking.payment_id" title="Pembayaran">
