<template>
  <div class="space-y-6">
    ...
    <!-- Recent bookings -->
    <AppCard title="Recent Bookings" :no-padding="true">
      <AppTable :columns="columns" :rows="tableRows" :loading="loading" clickable @row-click="goToBooking">
        ...
        <template #empty>
          <AppEmptyState title="No bookings yet" description="Bookings will appear here once customers start booking." />
        </template>
      </AppTable>
    </AppCard>
  </div>
...
const stats = computed(() => [
  { label: 'Active Bookings', value: data.value?.active_bookings ?? '—' },
  { label: "Today's Revenue", value: data.value ? formatCurrency(data.value.today_revenue) : '—' },
  { label: 'Units Rented', value: data.value?.units_currently_rented ?? '—' },
  { label: 'Total Customers', value: data.value?.total_customers ?? '—' },
  { label: 'Pending Payments', value: data.value?.pending_payments ?? '—' },
])
